...
Vec3 RectCloth::ComputeHookeForce(int iw_this, int ih_this,
   ...
   */
    ...
    float distance;
    size_t idx;
    if (!(Get1DIndex(iw_this, ih_this, idx)) || !(Get1DIndex(iw_that, ih_that, idx))) {
        return { 0, 0, 0 };
  }
    this_mass = local_or_world_positions[Get1DIndex(iw_this, ih_this)];
    that_mass = local_or_world_positions[Get1DIndex(iw_that, ih_that)];
    distance = length(this_mass - that_mass);
    force = (this_mass - that_mass) * stiffness * (dx_world - distance) / distance;
    return force;
...
Vec3 RectCloth::ComputeSpringForce(int iw, int ih) const {
   ...
   */
    ...
    float zf_dis;
    zf_dis = dx_local * scale[0] * scale[1] * scale[2];
    Vec3 force = { 0, 0, 0 };
    /*structural springs*/
    force += ComputeHookeForce(iw, ih, iw - 1, ih, zf_dis);
    force += ComputeHookeForce(iw, ih, iw + 1, ih, zf_dis);
    force += ComputeHookeForce(iw, ih, iw, ih - 1, zf_dis);
    force += ComputeHookeForce(iw, ih, iw, ih + 1, zf_dis);
    /*shear springs*/
    force += ComputeHookeForce(iw, ih, iw - 1, ih - 1, zf_dis * sqrt(2));
    force += ComputeHookeForce(iw, ih, iw + 1, ih - 1, zf_dis * sqrt(2));
    force += ComputeHookeForce(iw, ih, iw - 1, ih + 1, zf_dis * sqrt(2));
    force += ComputeHookeForce(iw, ih, iw + 1, ih + 1, zf_dis * sqrt(2));
    /*bending springs*/
    force += ComputeHookeForce(iw, ih, iw - 2, ih, zf_dis * 2);
    force += ComputeHookeForce(iw, ih, iw + 2, ih, zf_dis * 2);
    force += ComputeHookeForce(iw, ih, iw, ih - 2, zf_dis * 2);
    force += ComputeHookeForce(iw, ih, iw, ih + 2, zf_dis * 2);
    return force;
...
void RectCloth::LocalToWorldPositions() {
  ...
  for (int i = 0;i < mass_dim.x;i++) {
      for (int j = 0;j < mass_dim.y;j++) {
          local_or_world_positions[Get1DIndex(i, j)] = Transform::TransformPoint(local_or_world_positions[Get1DIndex(i, j)], model_matrix);
      }
  }
   ...
   */
    ...
    for (int i = 0;i < mass_dim.x;i++) {
        for (int j = 0;j < mass_dim.y;j++) {
            ...
            world_accelerations[Get1DIndex(i, j)] -= world_accelerations[Get1DIndex(i, j)] * damping_ratio / mass_weight;
        }
    }
}
...
void RectCloth::ComputeVelocities() {
   ...
   */
    for (int i = 0;i < mass_dim.x;i++) {
        for (int j = 0;j < mass_dim.y;j++) {
            if (is_fixed_masses[Get1DIndex(i, j)]) {
                world_velocities[Get1DIndex(i, j)] = { 0, 0, 0 };
            }
...
void RectCloth::WorldToLocalPositions() {
  ...
  for (int i = 0;i < mass_dim.x;i++) {
      for (int j = 0;j < mass_dim.y;j++) {
          local_or_world_positions[Get1DIndex(i, j)] = Transform::TransformPoint(local_or_world_positions[Get1DIndex(i, j)], glm::inverse(model_matrix));
      }
  }
}
